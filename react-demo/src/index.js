...
function Intro() {
    const element = (
        <div id="content">
            ...
            <h3 style={{ color: "tomato", fontFamily: 'Times New Roman', fontSize: 30, textAlign: "center" }} className="app">This app is created by {ModifyName(name)}</h3>
...
function Description(props) {
    const element = (
        <div style={{ marginTop: '20', width: '60%', height: '100%' }} >
            <p>It is a react app which implements the concept of component and property which is a feature of
                React JS. It is a <strong>JavaScript Library </strong> which is developed by {props.name} in <strong>{props.year}</strong>
            </p>
        </div>
    );
    return element;
...
function MovieDetails(props) {
    return (
        <div >
            <Poster posterUrl={props.movieDetail.PosterUrl} />
            <div className="details">
                <label>Name:</label>{props.movieDetail.Name}<br />
                <label>Release Date:</label>{props.movieDetail.ReleaseDate}<br />
                <label>Star Cast:</label>{props.movieDetail.Cast}<br />
            </div>
        </div>
    );
}
function Movie(props) {
    return (
        <div style={{ width: '200px' }} className="movie shadow">
            <React.Fragment>
                <MovieDetails movieDetail={props.movie}></MovieDetails>
            </React.Fragment>
        </div>
    );
}
